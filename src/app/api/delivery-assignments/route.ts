...
import {
  claimItem,
  listAvailableItemsPaged,
  listMyAssignmentsWithDetail,
} from "@/lib/db/deliveryDriverAssignment";
...
export async function POST(req: NextRequest) {
  ...
  if (targetUsername) {
    if (!hasAnyRole(auth.USER.ROLES, ["ADMIN"])) {
      return NextResponse.json(
        { error: "Hanya admin yang bisa assign ke user lain" },
        { status: 403 },
      );
    }
    ...
    if (!hasAnyRole(targetUser.ROLES, ["KURIR"])) {
      return NextResponse.json(
        { error: "User tersebut tidak punya role KURIR" },
        { status: 400 },
      );
    }
  ...
  try {
    const assignment = await claimItem(
      orderItemId,
      orderId,
      Number(quantity),
      deliveryDriver,
