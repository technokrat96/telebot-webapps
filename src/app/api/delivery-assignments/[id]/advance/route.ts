...
export async function PATCH(
  ...
  if (!Array.isArray(imageUrls) || imageUrls.length !== 1) {
    return NextResponse.json({ error: 'Wajib upload tepat 1 foto bukti sebelum ganti status' }, { status: 400 });
  }

  try {
    const ok = await advanceAssignmentDeliveryStatus(id, auth.TELEGRAM_USER, deliveryStatus, imageUrls);
    if (!ok) return NextResponse.json({ error: 'Assignment tidak ditemukan / bukan milik kamu' }, { status: 404 });
