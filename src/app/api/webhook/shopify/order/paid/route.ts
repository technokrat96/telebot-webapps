...
import { createTransaction, findTransactionByShopifyOrderId } from '@/lib/db/transaction';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Webhook belum dikonfigurasi (SHOPIFY_WEBHOOK_SECRET kosong)' }, { status: 500 });
  }
  if (!verified) {
  ...
  if (!order?.id || !Array.isArray(order.line_items) || order.line_items.length === 0) {
  ...
  if (existing) {
    return NextResponse.json({ ok: true, orderId: existing.ORDER_ID, duplicate: true });
  }
