...
import { listCurrencyCodes, updateCurrencyRate } from '@/lib/db/currency';
 ...
 * ExchangeRate-API. IDR di-skip (base currency kita, rate-nya selalu 1).
 *
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const codes = await listCurrencyCodes();
    ...
    for (const code of codes) {
      ...
      if (!rateFromIdr) {
        results.push({ code, error: 'kode tidak ditemukan di ExchangeRate-API' });
        continue;
      }
