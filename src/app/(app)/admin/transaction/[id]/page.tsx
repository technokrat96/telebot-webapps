...
function KurirProofPhotos({
  assignment,
}: {
  assignment: DeliveryDriverAssignment;
}) {
    ...
    <Space wrap size={12} style={{ marginTop: 4 }}>
      {assignment.IMAGE_URLS.map((url, idx) => (
        <Space key={url} orientation="vertical" size={2} align="center">
          <Tag
            color={DELIVERY_STATUS_COLORS[labels[idx]] ?? "default"}
            style={{ margin: 0 }}
          >
            {labels[idx] ?? "Foto"}
          </Tag>
...
function TransactionDetailContent() {
  ...
  const { data, isLoading, mutate } = useSWR<{
    transaction: TransactionWithDetailsAndAssignments;
  }>(`/api/transactions/${id}`, fetcher);
  ...
  async function submitAssign() {
    ...
    try {
      ...
      message.success(
        `Berhasil assign ${role === "FLORIST" ? "florist" : "kurir"}`,
      );
      closeAssignModal();
      await mutate();
    } catch (err) {
  ...
  async function releaseFloristAssignment(a: FloristAssignment) {
    ...
    try {
      await apiClient.patch(
        `/api/florist-assignments/${a.ASSIGNMENT_ID}/release`,
        {},
      );
  ...
  async function releaseDeliveryAssignment(a: DeliveryDriverAssignment) {
    ...
    try {
      await apiClient.patch(
        `/api/delivery-assignments/${a.ASSIGNMENT_ID}/release`,
        {},
      );
  ...
  return (
    <div>
      ...
      >
        ...
        <Space>
          <Button onClick={() => router.push("/admin/transaction")}>
            Kembali
          </Button>
          {!hasDeliveryPastPickup && (
            <Button
              type="primary"
              onClick={() => router.push(`/admin/transaction/${id}/edit`)}
            >
              Ubah Transaksi
            </Button>
          )}
      ...
      >
        {transaction && (
          <Descriptions column={2} bordered>
            ...
            <Descriptions.Item label="Sumber Order">
      ...
      <Space orientation="vertical" size={16} style={{ width: "100%" }}>
        {transaction?.details.map((item) => {
          const activeFlorist = item.assignments.filter(
            (a) => a.STATUS !== "RELEASED",
          );
          const activeKurir = item.deliveryAssignments.filter(
            (a) => a.STATUS !== "RELEASED",
          );
          ...
          return (
            ...
            >
              <Space orientation="vertical" size={4} style={{ width: "100%" }}>
                <Text>
                  Qty: {item.QUANTITY} · Harga satuan:{" "}
                  {(item.UNIT_PRICE || 0).toLocaleString("id-ID")} · Subtotal:{" "}
                ...
                {(item.CARD_TO || item.CARD_MESSAGE) && (
                  <Space orientation="vertical" size={0}>
                    ...
                    {item.CARD_MESSAGE && (
                      <Text type="secondary">Pesan: {item.CARD_MESSAGE}</Text>
                    )}
                  </Space>
                )}
                {item.RECEIVER_NAME && (
                  <Text type="secondary">
                    Penerima: {item.RECEIVER_NAME} · {item.RECEIVER_ADDRESS} ·{" "}
                    {item.RECEIVER_PHONE}
                  </Text>
                )}
              </Space>
              ...
              <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: 260 }}>
                  <Space align="center" style={{ marginBottom: 8 }}>
                    ...
                    <Tooltip
                      title={
                        canAssignFlorist
                          ? ""
                          : "Qty item ini sudah habis diambil florist"
                      }
                    >
                      <Button
                        type="primary"
                  ...
                  <div>
                    {activeFlorist.length === 0 && (
                      <Empty description="Belum ada florist" />
                    )}
                    <Space
                    ...
                    >
                      {activeFlorist.map((a) => (
                        <Space key={a.ASSIGNMENT_ID} size={4} wrap>
                          <Tag
                            color={
                              ASSIGNMENT_STATUS_COLORS[a.STATUS] ?? "default"
                            }
                          >
                            {ASSIGNMENT_STATUS_LABELS[a.STATUS] ?? a.STATUS}
                          </Tag>
                          <Text>
                            {a.FLORIST_NAME} · qty {a.QUANTITY_ASSIGNED}
                          </Text>
                          ...
                          {activeKurir.length === 0 && (
                            <Popconfirm
                              title="Lepas assignment ini?"
                              onConfirm={() => releaseFloristAssignment(a)}
                            >
                              <Button
                                danger
                ...
                <div style={{ flex: 1, minWidth: 260 }}>
                  <Space align="center" style={{ marginBottom: 8 }}>
                    ...
                    <Tooltip
                      title={
                        canAssignKurir
                          ? ""
                          : "Item harus berstatus READY TO PICKUP dulu sebelum bisa di-assign ke kurir"
                      }
                    >
                      <Button
                        type="primary"
                  ...
                  <div>
                    {activeKurir.length === 0 && (
                      <Empty description="Belum ada kurir" />
                    )}
                    <Space
                      orientation="vertical"
                      size={4}
                        ...
                        <div key={a.ASSIGNMENT_ID}>
                          <Space size={4} wrap>
                            ...
                            {a.DELIVERY_STATUS === "PICKUP" && (
                              <Popconfirm
                                title="Lepas assignment ini?"
                                onConfirm={() => releaseDeliveryAssignment(a)}
                              >
                                <Button
                                  danger
                                  loading={busyKey === a.ASSIGNMENT_ID}
        ...
        {transaction && transaction.details.length === 0 && (
          <Empty description="Tidak ada item" />
        )}
      </Space>

      <Modal
        title={
          assignTarget?.role === "FLORIST" ? "Assign Florist" : "Assign Kurir"
        }
        open={!!assignTarget}
      ...
      >
        {assignTarget && (
          <Space orientation="vertical" size={12} style={{ width: "100%" }}>
            ...
            <div>
              <Text>
                Pilih {assignTarget.role === "FLORIST" ? "florist" : "kurir"}:
              </Text>
              <Select
            ...
            <div>
              <Text>
                Qty (sisa {remainingQty(assignTarget.item, assignTarget.role)}):
              </Text>
              <InputNumber
                style={{ width: "100%", marginTop: 4 }}
                min={1}
