  ...
  return (
    <div>
      ...
      <Paragraph type="secondary">
        Pilih transaksi yang mau dibuatkan invoice. Transaksi yang sudah lunas
        ditagih (semua qty item sudah masuk invoice) tidak bisa dibuatkan
        invoice baru lagi.
      </Paragraph>
      <Table
        rowKey="ORDER_ID"
        ...
        columns={[
          ...
          { title: "Pelanggan", dataIndex: "CUSTOMER_NAME" },
          {
            ...
            width: 220,
            render: (_, r) => {
              const totalQty = r.details.reduce(
                (s, d) => s + Number(d.QUANTITY || 0),
                0,
              );
              const billedQty = r.details.reduce(
                (s, d) => s + Number(d.billedQty || 0),
                0,
              );
              const percent = totalQty
                ? Math.round((billedQty / totalQty) * 100)
                : 0;
              return (
                <Space
          ...
          },
          {
            ...
            render: (_, r) => (
              <Button
                ...
                onClick={() =>
                  router.push(`/admin/invoice/create/${r.ORDER_ID}`)
                }
              >
                Buat Invoice
              </Button>
