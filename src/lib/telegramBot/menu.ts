...
export function formatUsernameListMessage(items: {username: string; name: string}[], total: number): string {
  if (items.length === 0) {
    return 'Tidak ada user yang cocok.';
  }
  const list = items.map((u) => `<code>${u.username}</code>${u.name ? ` - ${u.name}` : ''}`).join('\n');
  const info =
    total > items.length
