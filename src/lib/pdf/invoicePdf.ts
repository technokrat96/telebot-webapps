...
export async function generateInvoicePdf(
  ...
  return new Promise((resolve, reject) => {
    ...
    doc
      .fontSize(10)
      .font("Helvetica")
      .text(data.INVOICE_NUMBER || data.INVOICE_ID, { align: "right" });
    doc.moveDown(1.5);
    ...
    doc
      ...
      .fillColor("#000")
      .font("Helvetica-Bold")
      .text(data.BILLED_TO || "-", 50, infoTop + 14);
    doc.fontSize(10).font("Helvetica").fillColor("#333");
    if (data.BILLED_ADDRESS)
      doc.text(data.BILLED_ADDRESS, 50, doc.y + 2, { width: 240 });
    if (data.BILLED_PHONE) doc.text(data.BILLED_PHONE, 50, doc.y + 2);

    doc
      .fontSize(9)
      .fillColor("#666")
      .text("ORDER ID", 350, infoTop, { width: 200, align: "right" });
    doc
      .fontSize(10)
      .fillColor("#000")
      .text(data.orderId || "-", 350, doc.y, { width: 200, align: "right" });
    doc
      .fontSize(9)
      .fillColor("#666")
      .text("TANGGAL INVOICE", 350, doc.y + 6, { width: 200, align: "right" });
    doc
      .fontSize(10)
      .fillColor("#000")
      .text(data.INVOICE_DATE?.slice(0, 10) || "-", 350, doc.y, {
        width: 200,
        align: "right",
      });
    doc
      .fontSize(9)
      .fillColor("#666")
      .text("JATUH TEMPO", 350, doc.y + 6, { width: 200, align: "right" });
    doc
      .fontSize(10)
      .fillColor("#000")
      .text(data.DUE_DATE?.slice(0, 10) || "-", 350, doc.y, {
        width: 200,
        align: "right",
      });
    ...
    for (const item of data.items) {
      const unitPrice = item.QUANTITY_BILLED
        ? item.PRICE_BILLED / item.QUANTITY_BILLED
        : 0;
      doc.text(item.ITEM_NAME, col.item + 6, y, { width: 240 });
      doc.text(String(item.QUANTITY_BILLED), col.qty, y, {
        width: 60,
        align: "right",
      });
      doc.text(formatIDR(unitPrice), col.price, y, {
        width: 80,
        align: "right",
      });
      doc.text(formatIDR(item.PRICE_BILLED), col.subtotal, y, {
        width: 80,
        align: "right",
      });
      y += 20;
    ...
    summaryRow(
      "Sisa",
      formatIDR(Math.max(0, data.TOTAL_AMOUNT - data.AMOUNT_PAID)),
      true,
    );
    ...
    doc
      .fontSize(8)
      .fillColor("#999")
      .text("Terima kasih atas kepercayaan Anda.", 50, 750, {
        align: "center",
        width: 495,
      });
