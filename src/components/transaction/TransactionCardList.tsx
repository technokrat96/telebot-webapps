...
import { TransactionDetailWithAssignments, TransactionWithDetailsAndAssignments } from '@/types';
...
const ITEM_STAGES = ['NEW ORDER', 'WORK IN PROGRESS', 'READY TO PICKUP', 'ON DELIVERY', 'DONE'];
// Satu warna beda per tahap, biar sekali lirik langsung ke-baca lagi di
// tahap mana tanpa perlu baca teksnya.
const ITEM_STAGE_COLORS = ['#8c8c8c', '#1677ff', '#fa8c16', '#722ed1', '#52c41a'];
 ...
 */
function hasAnyItemReadyToPickup(details: TransactionDetailWithAssignments[]): boolean {
  return details.some((d) => ITEM_STAGES.indexOf(d.ITEM_STATUS) >= READY_TO_PICKUP_INDEX);
}
 ...
 */
function hasDeliveryPastPickup(details: TransactionDetailWithAssignments[]): boolean {
  return details.some((d) =>
    d.deliveryAssignments.some((a) => a.STATUS !== 'RELEASED' && a.DELIVERY_STATUS !== 'PICKUP')
  );
}

const OFF_PIPELINE_STATUS_COLORS: Record<string, GetProp<typeof Tag, 'color'>> = {
  CANCELLED: 'red',
  PENDING: 'cyan',
  RESCHEDULED: 'gold',
};
 ...
 */
function ColorStepPanels({ steps, current }: { steps: PanelStep[]; current: number }) {
  return (
    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
      {steps.map((step, idx) => {
        ...
        return (
          ...
          >
            <div style={{ fontSize: 11, lineHeight: 1.3, whiteSpace: 'nowrap' }}>{step.label}</div>
            {step.description && (
              <div style={{ fontSize: 11, opacity: 0.9, marginTop: 2 }}>{step.description}</div>
            )}
          </div>
        );
      })}
 ...
 */
function summarizeDeliveryStatusQty(details: TransactionDetailWithAssignments[]) {
  let total = 0;
  let pickup = 0;
  let onDelivery = 0;
  let delivered = 0;
...
function QtyAssignPanels({
  ...
  const current = stage3Qty >= total ? 2 : stage3Qty > 0 || stage2Qty > 0 ? 1 : 0;
  const percent = current === 1 ? Math.round(((stage3Qty + stage2Qty) / total) * 100) : undefined;
  ...
  return (
    <div>
      ...
      <ColorStepPanels
        current={current}
        steps={[
          { label: stage1Label, color: stage1Color, description: `${stage1Qty} qty` },
          {
            label: stage2Label,
            color: stage2Color,
            description: `${stage2Qty} qty${current === 1 && percent !== undefined ? ` · ${percent}%` : ''}`,
          },
          { label: stage3Label, color: stage3Color, description: `${stage3Qty} qty` },
        ]}
      />
    </div>
  );
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <Space wrap style={{ marginBottom: 6 }}>
        ...
        {current === -1 && (
          <Tag color={OFF_PIPELINE_STATUS_COLORS[item.ITEM_STATUS] ?? 'default'}>{item.ITEM_STATUS}</Tag>
        )}
      </Space>
      {current !== -1 && (
        <ColorStepPanels
          current={current}
          steps={ITEM_STAGES.map((label, idx) => ({ label, color: ITEM_STAGE_COLORS[idx] }))}
        />
      )}
    </div>
  );
          ...
          return (
            ...
            >
              <Descriptions column={{ xs: 1, sm: 2, md: 4 }}>
                <Descriptions.Item label="Sumber">{t.ORDER_SOURCE || '-'}</Descriptions.Item>
                <Descriptions.Item label="Sales">{t.SALES_NAME || '-'}</Descriptions.Item>
              ...
              <Text strong style={{ display: 'block', marginBottom: 12 }}>Status Item</Text>
              {t.details.length === 0 ? (
                <Text type="secondary">Tidak ada item</Text>
              ) : (
                t.details.map((d) => <ItemStatusSteps key={d.ORDER_ITEM_ID} item={d} />)
              )}
