...
export default function TransactionForm({
  ...
  onSubmitAction: (values: TransactionFormValues) => void | boolean | Promise<void | boolean>;
  submitting?: boolean;
  isEdit?: boolean;
  ...
  async function handleFinish(values: TransactionFormValues) {
    ...
    try {
      resolvedDetails = await Promise.all(
        (values.details ?? []).map(async (d) => {
          ...
          return { ...rest, IMAGE_URLS: [...(rest.IMAGE_URLS ?? []), ...newUrls] };
        })
      );
    } catch (err) {
    ...
    const result = await onSubmitAction({ ...values, details: resolvedDetails });
    if (result === false) {
  ...
  function handleApplyOrderText() {
    ...
    if (parsed.CUSTOMER_NAME !== undefined) fieldsToSet.CUSTOMER_NAME = parsed.CUSTOMER_NAME;
    if (parsed.CUSTOMER_ADDRESS !== undefined) fieldsToSet.CUSTOMER_ADDRESS = parsed.CUSTOMER_ADDRESS;
    if (parsed.CUSTOMER_PHONE !== undefined) fieldsToSet.CUSTOMER_PHONE = parsed.CUSTOMER_PHONE;
    if (parsed.CUSTOMER_EMAIL !== undefined) fieldsToSet.CUSTOMER_EMAIL = parsed.CUSTOMER_EMAIL;
    if (parsed.DELIVERY_METHOD !== undefined) fieldsToSet.DELIVERY_METHOD = parsed.DELIVERY_METHOD;
    if (parsed.DELIVERY_DATE !== undefined) fieldsToSet.DELIVERY_DATE = parsed.DELIVERY_DATE as never;
    if (parsed.DELIVERY_TIME !== undefined) fieldsToSet.DELIVERY_TIME = parsed.DELIVERY_TIME as never;
    if (parsed.CARD_TO !== undefined) fieldsToSet.CARD_TO = parsed.CARD_TO;
    if (parsed.CARD_MESSAGE !== undefined) fieldsToSet.CARD_MESSAGE = parsed.CARD_MESSAGE;
    if (parsed.CARD_FROM !== undefined) fieldsToSet.CARD_FROM = parsed.CARD_FROM;

    const hasReceiverInfo = parsed.RECEIVER_NAME !== undefined
    ...
    message.success('Form terisi dari teks order. Cek Qty, Harga Satuan, dan Metode Pembayaran sebelum submit.');
  }

  function handleFinishFailed({errorFields}: ValidateErrorEntity<TransactionFormValues>) {
    console.log(errorFields)
    if (!errorFields.length) return;

    message.error('Ada input yang belum lengkap/valid, cek field yang ditandai merah.');
    ...
    setTimeout(() => {
      ...
      form.scrollToField(firstErrorName, {behavior: 'smooth', block: 'center'});
    ...
    >
      {!isEdit && (
        <Card style={{marginBottom: 16}}>
          ...
          <Form.Item label="Tempel teks order (format WA)" style={{marginBottom: 12}}>
            <Input.TextArea
              rows={6}
              value={orderText}
              onChange={(e) => setOrderText(e.target.value)}
              placeholder={"Thank you for placing your order!\nMohon diisi order form berikut:\nDetail PEMBELI:\nNama: ...\n..."}
            />
          </Form.Item>
          <Button onClick={handleApplyOrderText}>
            Parse &amp; Isi Form
          </Button>
        </Card>
      )}
      ...
      <Form.Item style={{ marginTop: 16 }}>
        <Button type="primary" htmlType="submit" loading={submitting || uploadingImages} block>
          {uploadingImages ? 'Mengupload foto…' : (isEdit ? 'Simpan Perubahan' : 'Buat Transaksi')}
        </Button>
      </Form.Item>
    </Form>
  );
