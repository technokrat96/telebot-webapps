...
async function openInvoicePdf(invoiceId: string) {
  const { url } = await apiClient.get<{ url: string }>(
    `/api/invoices/${invoiceId}/pdf-link`,
  );
  try {
...
export default function InvoiceListTable({
  ...
  async function handleSend() {
    ...
    try {
      await apiClient.post(
        `/api/invoices/${sendModalInvoice.INVOICE_ID}/send-telegram`,
        {
  ...
  return (
    <>
            ...
            render: (_, r) => (
              <Space.Compact block>
                <Button
                  ...
                  onClick={() =>
                    openInvoicePdf(r.INVOICE_ID).catch((err) =>
                      message.error((err as Error).message),
                    )
                  }
                >
      ...
      >
        <p>
          Invoice <strong>{sendModalInvoice?.INVOICE_NUMBER}</strong> akan
          dikirim sebagai dokumen PDF ke chat Telegram tujuan.
        </p>
        <Select
          style={{ width: "100%" }}
        ...
        <p style={{ marginTop: 8, color: "#999", fontSize: 12 }}>
          Cuma bisa kirim ke staff yang sudah pernah login ke app ini
          (Admin/Florist/Kurir). Belum bisa langsung ke nomor pelanggan.
        </p>
      </Modal>
    </>
  );
